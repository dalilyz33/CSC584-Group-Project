...
public class DonationDAO {
    ...
    public boolean approveDonation(String donationID, String staffID, java.sql.Date dateApproved) {
        String sql = "UPDATE Donation SET donation_status = 'Approved', staff_id = ?, donation_dateApproved = ? " +
                     "WHERE donation_id = ?";
        try (Connection con = com.careshare.util.DBConnection.createConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, staffID);
            ps.setDate(2, dateApproved);
            ps.setString(3, donationID);
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public boolean rejectDonation(String donationID, String staffID, java.sql.Date dateRejected) {
        String sql = "UPDATE Donation SET donation_status = 'Rejected', staff_id = ?, donation_dateApproved = ? " +
                     "WHERE donation_id = ?";
        try (Connection con = com.careshare.util.DBConnection.createConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, staffID);
            ps.setDate(2, dateRejected); // ERD shows donation_dateApproved is repurposed or left as review date
            ps.setString(3, donationID);
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
